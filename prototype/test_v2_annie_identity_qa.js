...
function assertAnnie(group,state){
  ...
  assert(group.getObjectByName('Annie_Stethoscope'),'Zhang-owned stethoscope missing');
  assert(group.getObjectByName('Annie_MouthAirway'),'CPR blow-training airway must remain visible');
  ...
  assert(group.getObjectByName('Annie_StethoscopeInscription'),'stethoscope inscription missing');
  assert.equal(group.userData.modelHeight,1.65,'standing reference height remains 165 cm');
  assert.equal(group.userData.inscription,'祝 守恆 醫師 1997 執業誌慶');
  let meshCount=0;
