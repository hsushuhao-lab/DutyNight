...
for(const id of ['first_campus_4f','second_campus_5f']){
 ...
 check(id+' nursing station has four oriented workstations',()=>{const work=zone.workstations.filter(w=>w.id.startsWith(prefix+'_station_'));assert.equal(work.length,4);for(const ws of work){assert(ws.desk);assert(Math.abs(ws.desk.rotation.y-ws.yaw)<.001);}});
