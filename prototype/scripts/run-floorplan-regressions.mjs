...
for(const file of tests){const p=spawnSync(process.execPath,[file],{encoding:'utf8',timeout:120000,maxBuffer:12*1024*1024});writeFileSync(`qa-results/node/${file}.log`,(p.stdout||'')+(p.stderr||''));const status=p.status===0?'PASS':'FAIL';results.push({file,status,exitCode:p.status,error:p.error?.message});console.log(status,file);if(status==='FAIL')console.log((p.stdout||'')+(p.stderr||''));}
