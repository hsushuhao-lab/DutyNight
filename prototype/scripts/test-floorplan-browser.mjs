...
await log('key / log / HIS via real UI');
 ...
 await go(-1.6,0);await shot('8f-card-gate',-Math.PI/2);await portal('BRIDGE_ACCESS','skybridge');await straight(30,0);await shot('bridge-middle',-Math.PI/2);await straight(58.4,0);await portal('BRIDGE_SECOND','second_campus_2f');await shot('second2-gate',Math.PI/2);await travel('second_campus_5f');await shot('second5-core',Math.PI);await go(72,3.2);await shot('second5-direct-entry',0);await card('second_ward');await go(72,.6);await shot('second5-inside-station',0);await go(69.7,-5.5);await shot('second5-staff-reader',0);await card('second_station_staff');await go(69.7,-8.5);await log('Second-campus entry -> central protected nursing station -> perimeter ward corridor');await roomTour();await go(72,-5.4);await shot('second5-station',0);await log('All 501-509 rooms visited');
