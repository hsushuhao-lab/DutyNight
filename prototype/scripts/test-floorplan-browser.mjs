...
await log('key / log / HIS via real UI');
 ...
 await go(-1.6,0);await shot('8f-card-gate',-Math.PI/2);await portal('BRIDGE_ACCESS','skybridge');await straight(30,0);await shot('bridge-middle',-Math.PI/2);await straight(58.4,0);await portal('BRIDGE_SECOND','second_campus_2f');await shot('second2-gate',Math.PI/2);await travel('second_campus_5f');await shot('second5-core',Math.PI);await go(72,3.2);await shot('second5-direct-entry',0);await card('second_ward');await go(72,1);await shot('second5-vestibule',0);await card('second_ward_inner');await go(72,-1.2);await shot('second5-ward-entry',0);await go(75,-.8);await shot('second5-staff-reader',0);await card('second_station_staff');await go(75,-3.4);await shot('second5-inside-station',Math.PI);await log('Second-campus dual-gate entry -> protected nursing station');await go(75,-.8);await card('second_station_staff',true);await roomTour();await go(72,-11.4);await shot('second5-station',Math.PI);await log('All 501-509 rooms visited with V5 duty room and entrance storage');
