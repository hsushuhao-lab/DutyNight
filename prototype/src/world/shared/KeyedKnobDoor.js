...
export class KeyedKnobDoor {
  constructor(zone,{id='duty_room',x,z,yaw=0,width=1.4,title='醫師值班室'}){
    ...
    this.leaf.userData={interactable:true,id,doorId:id,type:'duty_door',label:`鑰匙開啟${title}`};
    ...
    for(const side of [-1,1]){
      ...
      knob.position.set(width*.78,1.05,side*.09);this.hinge.add(knob);
      const plate=new THREE.Mesh(new THREE.CylinderGeometry(.09,.09,.018,20),knobMat);
      plate.rotation.x=Math.PI/2;plate.position.set(width*.78,1.05,side*.055);this.hinge.add(plate);
    }
  ...
  setClosed(closed){
    ...
    this.leaf.userData.label=closed?'鑰匙開門':'關門';
    this.root.updateWorldMatrix(true,true);
  }
