...
Revision: 2026-09-25
Status: `LOCAL_QA_PENDING / PUBLIC_QA_PENDING`
...
Current visual precedence: the direct user direction on 2026-09-25 overrides older pasted specifications and details depicted on reference boards. The face is nose-only with no eyes or mouth, Annie's chest is clear of attached props, and Zhang's stethoscope is staged separately. The archived user image [`annie-procedural-mood-reference-20260925.png`](art_reference/annie-procedural-mood-reference-20260925.png) informs adult silhouette, clothing, materials, pose, and atmosphere only; its depicted eyes, mouth/airway, and chest-worn stethoscope are not current design requirements.

## Read at a glance

Annie reads as a 165 cm clothing mannequin / old CPR training dummy in 張守恆's aged white coat. Use normal adult proportions and smooth, rounded Sphere/Capsule/Cylinder/Lathe/Torus geometry. Annie's body, coat, limbs, hands, and shoes must not use `BoxGeometry`. Her gray-ivory synthetic plastic, blank nose-only face, mold seams, rigid posture, and repeated presence carry the horror; do not pursue realistic skin or distort her face into a monster.

The dread comes from the mannequin's posture, CPR motion, absent facial features, repeated appearances, and wrong presence. Her face has one small molded nose only: no eyes, eye sockets, brows, pupils, lashes, mouth, or airway opening. Do not add facial marks or distort the nose into a monster feature.

The old yellowed white coat belongs to 張守恆. Annie has no chest badge, pockets, buttons, jewelry, pendant, or attached prop. A worn stethoscope rests separately beside the 3F storage training cart and appears again as the M5 bridge relic. Its metal name plate carries the exact inscription:
...
- Face: softly rounded vinyl shell with one small molded nose and fine mold seams; absolutely no eyes, eye sockets, brows, pupils, lashes, mouth, or airway.
...
- Chest: clear of any attached decoration, device, badge, pocket, or button. Zhang's separate stethoscope prop has worn rubber tubing, oxidized metal, and an inscription plate that visibly carries exactly `祝 守恆 醫師 1997 執業誌慶`.
...
- The face remains featureless apart from its small molded nose through every state. The horror is in what the body is doing and where it appears.

## Lighting and inspection

Keep the coat, blank vinyl face, nose, hinge seams, and CPR hand contact readable under local practical light. 6F may use dirty green/cyan spill, warm aged highlights on nearby metal, and deep surrounding shadow, while retaining the mannequin's face and CPR movement in view. Do not hide anatomy or defects by crushing the figure into black.

Browser evidence must include an M1 close inspection showing the molded nose-only face and clear chest, plus an M6 CPR wide shot and a closer CPR view. QA checks should verify the three shared states, static storage, active CPR movement, absent eyes/mouth/chest items, synthetic face material, and the exact inscription on the separate clue prop.
...
The shared implementation lives in `prototype/src/art/AnnieArt.js`. 3F storage, the skybridge, and 6F instantiate this factory with only `STORAGE_STATIC`, `BRIDGE_MANIFEST`, or `FLOOR6_CPR` changing the pose. It owns the model, materials, rig, light, and contact shadows. Zhang's stethoscope is a separate off-body clue prop. `prototype/src/world/shared/AnnieMannequin.js` is only an import bridge and contains no second model implementation.
...
- The face is smooth synthetic gray-ivory vinyl with one molded nose and no eyes or mouth. Neck/wrist seams, stiff joints, loose aged coat, gray-green scrubs, and worn shoes are visible at close range.
- Annie's chest is clear: no attached stethoscope, plate, badge, pockets, buttons, or other object. Zhang's engraved stethoscope remains a separate clue prop beside the storage cart and at the M5 bridge relic.
...
- The separate old stethoscope prop is individually inspectable and its exact inscription is readable in the M5 relic view.
...
Required Annie mesh nodes are `Annie_Face_Vinyl`, `Annie_MoldedNose`, `Annie_Hair_Wig`, `Annie_Coat`, `Annie_Scrubs`, `Annie_Shoe_L`, and `Annie_Shoe_R`. Do not include eye, pupil, eyelash, mouth, or airway nodes on Annie. Zhang's separate prop uses the `Zhang_Stethoscope_1997` root and independently addressable inscription plate. Required material names are `Annie_Mat_Vinyl`, `Annie_Mat_Coat`, `Annie_Mat_Scrubs`, `Annie_Mat_Stethoscope_Rubber`, and `Annie_Mat_Stethoscope_Metal`. The prop must remain independently loadable and must not be parented to the Annie skeleton.
...
The triangle budget counts rendered triangles after triangulation across the character's visible LOD. The named blank face, nose, hands, clothing, and shoes must remain present in the optimized export. The separate stethoscope prop keeps its own optimized asset and provenance. Current asset sizes are measured from the exact files fetched by the browser, including any external texture dependencies; do not report only the geometry chunk size.
...
Keep shoulder, elbow, and wrist behavior deliberately stiff. If finger bones are included, thumb and palm articulation are the minimum useful control. Facial blendshapes are not required; the face has no eyes or mouth and must not emote. The separate stethoscope chestpiece must remain inspectable for M5 clue continuity and must not be attached to Annie.
...
Author at 30 samples per second with seconds-based glTF key times. All loop clips must have matching first/last poses and no visible pop over ten repeated loops. No eye or mouth meshes, facial animation, eye darts, blinks, or gaze-target constraints are permitted. If the head-track clip is used, it starts 250–450 ms after its proximity trigger, turns slowly to at most ±35° yaw, and never snaps. Cross-fades between non-loop poses complete within 0.15–0.30 seconds.
...
- `STORAGE_STATIC` in 3F: seated still; coat, scrub underlayer, and shoes present; no chest accessories. The separate stethoscope rests beside the training cart.
- `BRIDGE_MANIFEST` in M5: distant first read, nose-only blank face, empty-CPR action. The same rig, silhouette, coat, face, and hair must identify her as the previous mannequin.
- `FLOOR6_CPR` in M6: kneeling beside the scorched bed and performing full CPR on the human-shaped burn silhouette.

Do not build separate white figures in zone files. `Skybridge.js` and `Phantom6F.js` may choose a state and transform, but may not create Annie's body, face, or clothing from scene-local primitive geometry. Scene clue props remain detached from the character.

## Lighting and camera acceptance

Keep the existing story lighting architecture. Skybridge remains navigable with reduced fill; when Annie appears, use a localized cool-white practical/spot accent from the scene's existing lighting system to reveal the blank nose-only face, coat dirt, mold seam, and plastic highlight. Use cast shadows on Annie and contact shadows under the feet/knees; floor receives shadows. Do not add an isolated studio rig or brighten the entire scene with flat ambient light.
...
- CLOSE: 0.8–1.5 m, clearly shows smooth vinyl, the single molded nose, uninterrupted blank face, fine mold seam, coat cloth detail, and empty chest.
...
For each capture, record the source commit, zone/state, camera-to-anchor distance, camera transform, viewport, and screenshot SHA-256 in the QA report. The long and mid shots validate silhouette and staging; close shots must show molded vinyl with a single nose, no eyes or mouth, fine seam, cloth wear, and a clear chest. The separate stethoscope relic inspection must show the exact inscription `祝 守恆 醫師 1997 執業誌慶` without substituted or malformed characters. These Annie views are included in the 26-image story manifest; there is no separate asset screenshot gate.
